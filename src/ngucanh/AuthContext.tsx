...
type AuthContextValue={user:User|null;loading:boolean;login:(login:string,password:string)=>Promise<void>;register:(data:object)=>Promise<void>;logout:()=>Promise<void>;refresh:()=>Promise<void>};
...
export function AuthProvider({children}:PropsWithChildren){const [user,setUser]=useState<User|null>(null);const [loading,setLoading]=useState(true);
 ...
 const save=async(data:{token:string;user:User})=>{await tokenStorage.set(data.token);setUser(data.user);};
 return <AuthContext.Provider value={{user,loading,login:async(login,password)=>save((await authService.login({login,password})).data),register:async(data)=>save((await authService.register(data)).data),logout:async()=>{await tokenStorage.remove();setUser(null);},refresh}}>{children}</AuthContext.Provider>}
