...
export function AuthProvider({children}:PropsWithChildren){const [user,setUser]=useState<User|null>(null);const [loading,setLoading]=useState(true);
 ...
 return <AuthContext.Provider value={{user,loading,login:async(login,password,remember=true)=>save((await authService.login({login,password})).data,remember),register:async(data)=>save((await authService.register(data)).data),logout:async()=>{await tokenStorage.remove();setUser(null);},refresh}}>{children}</AuthContext.Provider>}
